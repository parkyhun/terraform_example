...
terraform {
  ...
  required_providers {
    ...
    tailscale = {
        source = "tailscale/tailscale"  # 정해진거
        version = "0.17.2"
    }
  }
}
...
resource "aws_instance" "nat_ec2" {
    ...
    subnet_id              = aws_subnet.public_subnet.id
    # 외부에서 나가야 하기 때문에 public ipv4 주소 필요
    associate_public_ip_address = true
    vpc_security_group_ids = [aws_security_group.nat_sg.id]
    key_name               = aws_key_pair.kp.key_name
    # 다른 ec2에서 받은 트래픽을 외부에 전달해야 하기 때문에 false로 설정
...
resource "aws_route_table" "private_rt" {
    ...
    route {
        ...
        #nat_gateway_id = aws_nat_gateway.nat_gw.id  #게이트웨이 일대 사용한 명령어
...
data "tailscale_device" "my_ec2_device" {
  ...
  wait_for   = "180s" # 대신 위의 대장 ec2 가 리스트에 뜰 때까지 테라폼이 기다려줍니다.

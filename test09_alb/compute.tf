...
resource "aws_instance" "my_ec2" {
    ...
    # count.index % 2 사용하면 0,1,0,1,0... 순서로 리스트에 있는 값이 적용된다
    subnet_id = [aws_subnet.public_subnet_1.id, aws_subnet.public_subnet_2.id][count.index % 2] # 2개의 퍼블릭 서브넷을 번갈아가며 사용
